...
/******************* 必须要改！！不然得不到天气！！ ******************/
const hefengKey = "772e75882af3485789bc6d8c7fb9007a"; //  和风天气Web api的key

/******************* 可能需要你修改的部分 ******************/
const mqttHost = "mqtt.mqttasd.xyz"; //mqtt 服务器域名/IP
...
export default {
  ...
  onShow() {
    var that = this;
    const latitude = 30.0483;
    const longitude = 103.832;
    const key = hefengKey;
    wx.showToast({
      title: "连接服务器....",
      icon: "loading",
...
.wrapper .data-wrapper .data .data-logo {
  height: 36px;
  width: 36px;
  margin-top: 15px;
}
.wrapper .data-wrapper .data .data-text {
  margin-top: 15px;
  color: #7f7f7f;
}
.wrapper .data-wrapper .data .data-text .data-title {
  text-align: right;
}
.wrapper .data-wrapper .data .data-text .data-value {
  font-size: 26px;
}
...
.wrapper .header-wrapper .header-text {
  font-size: 32px;
  font-weight: 400;
  display: flex;
  justify-content: space-between;
}
.wrapper .header-wrapper .weather-advice {
  margin-top: 20px;
  font-size: 12px;
}
</style>
